...
func (key *PrivateKey) Encrypt(m *big.Float) (c *big.Float, err error) {
...
func (key *PublicKey) Encrypt(m *big.Float) (c *big.Float, err error) {
...
func (key *PrivateKey) Decrypt(c *big.Float) (m *big.Float, err error) {
	...
	// where L(x) = (x-1)/n
	//m = new(big.Float).Exp(c, key.Lambda, key.PublicKey.NSquared)
...
// of a ciphertext.
//func getL(x, n *big.Float) (ans *big.Float) {
//	ans = new(big.Float).Div(new(big.Float).Sub(x, one), n)
//	return ans
//}
