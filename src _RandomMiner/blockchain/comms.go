...
func (c *Chain) GetFormat() election.Format {
	//fmt.Println(c.conf.ElectionFormat)	
...
func (c *Chain) ReceiveTransaction(t *Transaction, _ *struct{}) (err error) {
	...
	for _, tr := range pool {
		if (reflect.DeepEqual(t.Header.Signature.R.String(), tr.Header.Signature.R.String())) {
			c.SeenTrs <- seen
			c.TransactionPool <- pool
			//fmt.Println("Seen Transaction from "+t.Header.VoteToken)
			return nil
		}
	}
